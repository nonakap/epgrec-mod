...
//						rename( $xmlfile, $new_name );
				...
				if( $type !== 'GR' ){
					...
					if( strcmp( $rec->channel, $tmp_ch ) ){
						...
						if( !((boolean)$rec->skip)  ){
							// 既予約の物理チャンネル番号を変更
							$options = "WHERE complete = '0' AND channel_id = '".$rec->id."'";
							$hit     = DBRecord::countRecords( RESERVE_TBL, $options );
							if( $hit > 0 ) {
								$revs = DBRecord::createRecords( RESERVE_TBL, $options );
								foreach( $revs as $rev ){
									if( (int)$rev->autorec ){
										// 自動キーワードはキャンセルのみ
										...
										Reservation::cancel( $rev->id );
										Reservation::custom( $st_tm, $ed_tm, $ch_id, $title, $desc, $cat_id, $prg_id, 0, $rs_md, $discon, $rs_dt, $prior );
									}
								}
								unset( $rev );
							}
...
//			rename( $xmlfile, $new_name );
		...
		if( $ev_lmt == 0 ){
			...
			$options = "WHERE channel_disc = '".$channel_disc."' ".
				"AND starttime < '".toDatetime($epg_end)."' AND endtime > subtime( '".$event_pf[0]['starttime']."', '03:00:00' )";
			$tmp_lmt = DBRecord::countRecords( PROGRAM_TBL, $options );
			if( $tmp_lmt > 0 ){
				$records = DBRecord::createRecords( PROGRAM_TBL, $options.' ORDER BY starttime ASC' );
				$pf_trim = 0;
					...
					if( $event_pf[$pf_cnt]['sch_pnt'] < 0 ){
						...
						if( $hit > 0 ){
							...
							if( $pf_cnt == 0 ){
								...
								$options = "WHERE channel_disc = '".$channel_disc."' AND starttime < '".$sch[0]->starttime."' AND endtime >= '".$sch[0]->starttime."'";
								$hit = DBRecord::countRecords( PROGRAM_TBL, $options );
								if( $hit > 0 ){
									// 複数の場合を未考慮
									$sch_in = DBRecord::createRecords( PROGRAM_TBL, $options.' ORDER BY endtime DESC' );
									//番組挿入
							...
							if( $pf_cnt == 0 ){
								//EIT[pf]より前が無いEIT[sch]をDBより１つ補完
								$options = "WHERE channel_disc = '".$channel_disc."' AND starttime < '".$event_pf[$pf_cnt]['starttime']."' AND endtime >= '".$event_pf[$pf_cnt]['starttime']."'";
								$hit = DBRecord::countRecords( PROGRAM_TBL, $options );
								if( $hit > 0 ){
									// 複数の場合を未考慮
									$sch_in = DBRecord::createRecords( PROGRAM_TBL, $options.' ORDER BY endtime DESC' );
						...
						if( $pf_cnt == 0 ){
							if( $event_pf[$pf_cnt]['sch_pnt'] == $ev_cnt+1 ){
								...
								if( $hit > 0 ){
									// 複数の場合を未考慮
									$sch = DBRecord::createRecords( PROGRAM_TBL, $options.' ORDER BY endtime DESC' );
									//番組挿入
									$event_add[$ev_cnt]['starttime']    = $sch[0]->starttime;
									$event_add[$ev_cnt]['endtime']      = $sch[0]->endtime;
									$event_add[$ev_cnt]['channel_disc'] = $channel_disc;
									...
									if( time() >= $rec_start - $settings->former_time ) {
										...
										|| ( $type==='EX' && $EX_TUNERS_CHARA[$prev_tuner]['cntrl'] )
											...
											if( $add_time > 0 ){
												...
												if( $now_bat != 0 ){
													// ギリギリまで延伸
													$bat_rev  = DBRecord::createRecords( RESERVE_TBL, $sql_cmd."' AND starttime <= '".$new_end."' ORDER BY starttime ASC" );
...
//										reclog( $rev_ds.'は時間変更の可能性があり予約取り消し' );
				...
				reclog( 'EPG更新:: プログラムテーブルに問題が生じた模様', EPGREC_ERROR );
				reclog( 'EPG更新:: '.$e->getMessage() , EPGREC_ERROR);
				return -1;
			}
		}
		unset( $program );
...
*/
		...
		foreach( $stk_rev as $post ){
			try{
				$options = "WHERE channel_id = '".$post['ch_id']."' AND eid = '".$post['eid']."'";
				$hit = DBRecord::countRecords( PROGRAM_TBL, $options );
				if( $hit > 0 ) {
					$prg = DBRecord::createRecords( PROGRAM_TBL, $options );
					if( $post['ch_id'] == (int)($prg[0]->channel_id) ){
						...
						try{
							Reservation::custom( $st_tm, $ed_tm, $post['ch_id'], $post['title'], $post['desc'], $post['cat_id'], (int)$prg[0]->id,
			...
			try{
				Reservation::custom( $st_tm, $ed_tm, $post['ch_id'], 'X_'.$post['title'], $post['desc'], $post['cat_id'], 0,
											0, $post['rs_md'], $post['discon'], $post['rs_dt'], $post['prior'] );
			}catch( Exception $e ){
